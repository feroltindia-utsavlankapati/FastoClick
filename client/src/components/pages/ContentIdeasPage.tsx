                  ...
                  {isExpanded && (
                    <div className="border-t border-[#d0d7e3] px-6 pb-7 pt-5">
                      ...
                      <div className="flex gap-2 mb-6 flex-wrap">
                        {TABS.map(({ key, label, icon: Icon }) => (
                          <button key={key} onClick={() => setActiveTab(key)}
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === key ? "soft-btn-primary" : "soft-btn"}`}>
                            <Icon size={14} />{label}
                          </button>
                        ))}
                      </div>
                      ...
                      {activeTab === "tone" && record.result?.tone_style && (() => {
                        const ts = record.result.tone_style;
                        return (
