            ...
            <main className="flex-1 max-w-7xl mx-auto w-full p-6 md:p-10 relative z-10 flex flex-col gap-8">
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 mt-2 font-medium">Use these dynamic variables to personalize your automated emails.</p>
                    </div>
                </header>

                <div className="bg-white border border-slate-200 shadow-sm rounded-xl rounded-[32px] p-8 mb-4 border-l-4 border-[#8B5CF6] flex gap-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white border border-slate-200 shadow-sm rounded-xl rounded-[32px] p-8 flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-4">
                            {placeholders.map((ph, idx) => (
                                <div key={idx} className="bg-slate-50 border border-slate-200 rounded-xl p-5 rounded-2xl flex flex-col gap-3 group">
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-2">{ph.category}</span>
                                        </div>
                                        <button 
                                            onClick={() => copyToClipboard(ph.tag)}
                                            className="p-2 rounded-lg text-slate-500 hover:bg-white hover:shadow-sm transition-all"
                                            title="Copy to clipboard"
                                        >
                                            {copiedTag === ph.tag ? <CheckCircle2 size={16} className="text-success" /> : <Copy size={16} />}
                                        </button>
                                    </div>
                                    <p className="text-sm font-medium text-slate-900">{ph.description}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Example Usage */}
                    <div className="bg-white border border-slate-200 shadow-sm rounded-xl rounded-[32px] p-8 flex flex-col gap-6 h-fit">
                        <h2 className="text-xl font-bold flex items-center gap-2 mb-2">
                        ...
                        <div className="p-6 rounded-2xl border-2 border-dashed border-[#A0AEC0] flex flex-col gap-4 bg-white/30">
                            <div className="text-sm font-bold text-slate-500 uppercase tracking-wider">How the recipient sees it:</div>
