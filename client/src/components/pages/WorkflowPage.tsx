...
const STATUS_CONFIG = {
  QUEUED:    { label: "Queued",    color: "text-[#A0AEC0]",  bg: "",                        icon: Clock },
  RUNNING:   { label: "Running",   color: "text-[#6C63FF]",  bg: "bg-[#6C63FF]/5",          icon: Loader2 },
...
export default function WorkflowPage() {
  ...
  return (
    <div className="min-h-screen bg-[#E0E5EC] text-[#3D4852] font-sans flex flex-col relative overflow-hidden">
      <NavigationBar />

      {/* Decoration */}
      <div className="absolute top-20 right-10 w-96 h-96 rounded-full soft-inset pointer-events-none opacity-20">
        <div className="w-64 h-64 rounded-full soft-extruded m-16" />
      </div>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-6 md:p-10 relative z-10">
        <header className="mb-10">
          ...
          <p className="text-[#6B7280] font-medium">Trigger multi-agent workflows and monitor live execution.</p>
        </header>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">

          {/* ── LEFT: Workflow card ──────────────────────────────────────── */}
          <div className="soft-extruded rounded-[32px] p-8 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-[#6C63FF] text-sm font-extrabold">On-Demand</p>
                </div>
              </div>
              ...
              {/* Agents */}
              <div className="soft-inset p-5 rounded-2xl mb-5">
                <div className="text-xs text-[#6B7280] mb-3 uppercase font-extrabold tracking-wider">Agents</div>
                <div className="flex flex-wrap gap-3">
                  {["Strategy Planner", "SEO Specialist"].map(a => (
                    <span key={a} className="flex items-center gap-2 px-3 py-1.5 soft-extruded-sm rounded-xl text-xs font-bold">
                      <Bot size={14} className="text-[#6C63FF]" />{a}
                    </span>
                  ))}
                </div>
              </div>
              ...
              {/* Steps */}
              <div className="soft-inset p-5 rounded-2xl">
                <div className="text-xs text-[#6B7280] mb-3 uppercase font-extrabold tracking-wider">Workflow Steps</div>
                <ol className="space-y-2">
                  {[
                    ...
                    <li key={i} className="flex items-center gap-3 text-sm font-medium text-[#6B7280]">
            ...
            <button
              onClick={handleStart}
              disabled={executing}
              className="mt-8 w-full py-4 soft-btn-primary rounded-2xl font-bold text-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
            >
              {executing
          ...
          <div className="space-y-6">
            <div className="soft-extruded rounded-[32px] p-8">
              ...
              {/* Poll error banner */}
              {pollError && (
                <div className="soft-inset rounded-2xl p-4 mb-5 flex items-start gap-3 text-sm font-medium text-red-500">
              ...
              {job ? (
                <div className="space-y-5">
                  {/* Status */}
                  <div className={`soft-inset rounded-2xl p-5 ${statusCfg?.bg ?? ""}`}>
                    <div className="text-xs font-extrabold text-[#6B7280] uppercase tracking-wider mb-3">Status</div>
                  ...
                  {/* Meta */}
                  <div className="soft-inset rounded-2xl p-5 space-y-3">
                    <div>
                      <div className="text-xs font-extrabold text-[#6B7280] uppercase tracking-wider mb-1">Workflow ID</div>
                      <div className="font-mono text-xs text-[#A0AEC0] break-all">{job.workflow_id}</div>
                    </div>
                    <div className="flex gap-6">
                      <div>
                        <div className="text-xs font-extrabold text-[#6B7280] uppercase tracking-wider mb-1">Started</div>
                        <div className="text-sm font-medium">{fmt(job.started_at)}</div>
                      </div>
                      {job.finished_at && (
                        <div>
                          <div className="text-xs font-extrabold text-[#6B7280] uppercase tracking-wider mb-1">Finished</div>
                  ...
                  {/* Error detail */}
                  {job.error && (
                    <div className="soft-inset rounded-2xl p-4 text-sm text-red-500 font-medium flex items-start gap-2">
                      <AlertCircle size={16} className="flex-shrink-0 mt-0.5" />
                      {job.error}
                    </div>
                  )}
                ...
                <div className="flex flex-col items-center justify-center text-center p-8 soft-inset rounded-2xl min-h-[180px]">
                  <div className="w-16 h-16 rounded-full soft-inset-deep flex items-center justify-center mb-4 animate-float">
                    <Radio size={28} className="text-[#6B7280]" />
                  </div>
                  <p className="text-[#6B7280] font-bold">Trigger a workflow to see live status here.</p>
                  <p className="text-[#A0AEC0] text-sm mt-1 font-medium">No Temporal required.</p>
                </div>
              )}
            </div>
            ...
            {/* ── Inline Plan Result ──────────────────────────────────── */}
            {planResult?.plan && (
              <div className="soft-extruded rounded-[32px] p-8 space-y-5">
                {/* Header */}
                      ...
                      <div className="soft-inset rounded-2xl p-4">
                        ...
                        <ul className="space-y-2">
                          {planResult.plan.strategy.core_strategy!.map((item, i) => (
                            <li key={i} className="flex items-start gap-3 text-sm font-medium text-[#6B7280]">
                              <span className="w-5 h-5 rounded-md soft-inset-sm flex items-center justify-center text-[#6C63FF] text-xs font-extrabold flex-shrink-0 mt-0.5">
                                {i + 1}
                              </span>
                              {item}
                            </li>
                ...
                {(planResult.plan.campaign_plans?.phases ?? []).length > 0 && (
                  <div className="soft-inset rounded-2xl p-4">
                    <div className="text-xs font-extrabold text-[#6B7280] uppercase tracking-wider mb-3 flex items-center gap-2">
                      <LayoutList size={13} /> Execution Phases
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      {planResult.plan.campaign_plans!.phases!.map((ph, i) => (
                        <div key={i} className="soft-inset-sm rounded-xl p-3">
                          <p className="text-xs font-extrabold text-[#6C63FF] mb-0.5">{ph.duration}</p>
                          <p className="text-xs font-medium text-[#6B7280] truncate">{ph.phase}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="soft-inset rounded-2xl p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {planResult.plan.campaign_plans!.channels!.map((ch, i) => (
                        <span key={i} className="px-3 py-1.5 soft-inset-sm rounded-xl text-xs font-bold text-[#6C63FF]">
                          {ch.channel}
                        </span>
                      ))}
                    </div>
                ...
                <button
                  ...
                  className="w-full py-3 soft-btn-primary rounded-2xl font-bold text-sm flex items-center justify-center gap-2"
                >
