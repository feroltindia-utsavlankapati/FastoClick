            ...
            <main className="flex-1 max-w-7xl mx-auto w-full p-6 md:p-10 relative z-10 flex flex-col gap-8">
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-[#6B7280] mt-2 font-medium">Create and track your email broadcasts and sequences.</p>
                    </div>
                    {!isCreating && (
                        <button 
                            onClick={() => {
                ...
                {isCreating ? (
                    <div className="soft-extruded rounded-[32px] p-8 flex flex-col gap-8 animate-in slide-in-from-bottom-4 duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="flex flex-col gap-6">
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-bold text-[#6B7280]">Campaign Name</label>
                                    <input 
                                        type="text" 
                                        value={newCampaign.name}
                                        onChange={(e) => setNewCampaign({...newCampaign, name: e.target.value})}
                                        className="p-4 bg-transparent soft-inset rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-[#EAB308]"
                                        placeholder="e.g. Q4 Newsletter"
                                    />
                                </div>
                                
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-bold text-[#6B7280]">Select Template</label>
                                    <select 
                                        value={newCampaign.template_id}
                                        onChange={(e) => setNewCampaign({...newCampaign, template_id: e.target.value})}
                                        className="p-4 bg-transparent soft-inset rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-[#EAB308] appearance-none"
                                    >
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-bold text-[#6B7280]">Sender Email ("From" Address)</label>
                                    <input 
                                        type="email" 
                                    ...
                                    />
                                    <p className="text-xs text-[#6B7280]">Leave blank to use default system email.</p>
                                </div>

                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-bold text-[#6B7280]">Schedule (Optional)</label>
                                    <input 
                                        type="datetime-local" 
                                        ...
                                        className="p-4 bg-transparent soft-inset rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-[#EAB308]"
                                    />
                                    <p className="text-xs text-[#6B7280]">Leave blank to send immediately.</p>
                                </div>
                            </div>

                            <div className="flex flex-col gap-2 h-[400px]">
                                <label className="text-sm font-bold text-[#6B7280] flex justify-between">
                                ...
                                <div className="flex-1 soft-inset rounded-2xl p-4 overflow-y-auto flex flex-col gap-2">
                                    {contacts.length === 0 ? (
                                        <div className="text-sm text-[#6B7280] text-center mt-10">No contacts available.</div>
                                    ) : (
                                        contacts.map(c => (
                                            <div 
                                                key={c.id} 
                                                onClick={() => toggleContact(c.id)}
                                                className={`p-3 rounded-xl cursor-pointer border flex items-center justify-between transition-all ${newCampaign.contact_ids.includes(c.id) ? "border-[#EAB308] bg-[#E0E5EC]/50 shadow-sm" : "border-transparent soft-btn"}`}
                                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {loading ? (
                            <div className="col-span-full py-20 text-center text-[#6B7280] font-bold">Loading campaigns...</div>
                        ) : campaigns.length === 0 ? (
                            ...
                            campaigns.map(campaign => (
                                <div key={campaign.id} className="soft-extruded rounded-[32px] p-6 flex flex-col justify-between h-48">
                                    <div>
                                        ...
                                        <h3 className="text-lg font-bold text-[#3D4852] mb-1 truncate">{campaign.name}</h3>
                                        <p className="text-sm text-[#6B7280] truncate font-medium flex items-center gap-1">
                                    ...
                                    <div className="flex justify-between items-center text-xs text-[#6B7280] font-bold mt-4 pt-4 border-t border-white/20">
                                        <a href={`/email/analytics`} className="text-[#6C63FF] hover:underline">View Analytics &rarr;</a>
