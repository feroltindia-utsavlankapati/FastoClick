        ...
        <>
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        ...
                        <p className="text-[#6B7280] mt-2 font-medium">Visualize your scheduled email campaigns</p>
                    </div>
                    <button
                    ...
                    <div className="lg:col-span-3">
                        <div className="soft-extruded rounded-[32px] p-6">
                            ...
                            <div className="grid grid-cols-7 gap-2 mb-2">
                                {DAYS.map(day => (
                                    <div key={day} className="text-center text-xs font-bold text-[#6B7280] py-2">{day}</div>
                                ))}
                            </div>
                                ...
                                {calendarDays.map((day, i) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`soft-calendar-cell min-h-[100px] cursor-pointer flex flex-col p-2 ${isToday ? "today" : ""} ${isSelected ? "soft-inset" : "hover:soft-btn"}`}
                                        >
                                            <div className={`text-xs font-bold mb-1 ${isToday ? "text-[#6C63FF]" : "text-[#6B7280]"}`}>{day}</div>
                                            {hasCampaigns && (
                                            ...
                                            {hasCampaigns && (
                                                <div className="hidden md:flex flex-col gap-1 mt-auto w-full">
                                                    {campaignsByDate[dStr].slice(0, 2).map((c, idx) => (
                                                        <div key={idx} className="text-[10px] truncate bg-white/50 px-1.5 py-0.5 rounded font-medium text-[#3D4852]">
                                                            {c.title}
                                                        </div>
                                                    ))}
                                                    {campaignsByDate[dStr].length > 2 && (
                                                        <div className="text-[9px] text-[#6B7280] font-bold">+{campaignsByDate[dStr].length - 2} more</div>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div className="soft-extruded rounded-[28px] p-6 sticky top-28">
                            ...
                            {!selectedDate ? (
                                ...
                                <div className="text-center py-6">
                                    ...
                                    <button onClick={() => navigate(`/projects/${projectId}/email/campaigns`)} className="px-4 py-2 soft-btn-primary rounded-xl text-xs font-bold">
                                ...
                                <div className="space-y-3">
                                    {selectedCampaigns.map((camp) => (
                                        <div key={camp.id} className="soft-inset rounded-xl p-3">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${camp.status === 'scheduled' ? 'bg-[#638DFF]/15 text-[#638DFF]' : 'bg-[#6B7280]/15 text-[#6B7280]'}`}>
                                                    {camp.status}
                                                </span>
                                            </div>
